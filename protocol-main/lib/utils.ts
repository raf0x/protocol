...
export function isDueToday(frequency: string, protocolStart: string, dayOfWeek: number | null, checkDateStr?: string, daysOfWeek?: number[]): boolean {
  ...
  // Check rolling cycles first (before days_of_week)
  ...
  if (frequency === 'every6days') return daysDiff % 6 === 0
  if (frequency === 'every7days') return daysDiff % 7 === 0
  
  // For weekly patterns, use days_of_week array
  if (daysOfWeek && daysOfWeek.length > 0) return daysOfWeek.map(Number).includes(todayDay)
  
  // Legacy frequency strings

...
export default function QuickProtocolFields({ value, onChange }: { value: Fields; onChange: (field: string, value: string | boolean | number[]) => void }) {
  ...
  const unit = (field: 'vial_unit' | 'dose_unit', label: string) => <label>{label}<select aria-label={label} value={value[field]} onChange={event => onChange(field, event.target.value)}>{['mg','mcg','IU'].map(unit => <option key={unit}>{unit}</option>)}</select></label>
  return <div className="protocol-quick-fields">
    ...
    <label>Time of day<select aria-label="Time of day" value={value.time_of_day} onChange={event => onChange('time_of_day',event.target.value)}>{['Morning','Afternoon','Evening','Night'].map(time => <option key={time}>{time}</option>)}</select></label>

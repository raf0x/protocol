...
export default function ProtocolLibrary(props: Props) {
  return <div className="protocol-library">{['Active', 'Planned', 'Completed'].map(section => {
    const rows = props.protocols.filter(protocol => section === 'Completed' ? protocol.status === 'completed' : section === 'Planned' ? protocol.status === 'planned' : protocol.status !== 'completed' && protocol.status !== 'planned')

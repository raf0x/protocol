...
export default function TimelineBaseline({ baseline }: { baseline: CurrentBaseline }) {
  return <section className={styles.baseline} aria-labelledby="baseline-title">
    ...
    <dl className={styles.baselineKpis}>
      <div className={styles.kpi}>
        <dt>Latest weight</dt>
        <dd>{baseline.weight != null ? <>{baseline.weight}<small> lbs</small></> : 'Not recorded'}</dd>
        {baseline.weightDate ? <time dateTime={baseline.weightDate}>{formatTimelineDate(baseline.weightDate)}</time> : <span>No weight yet</span>}
      </div>
      <div className={styles.kpi}>
        <dt>Active protocols</dt>
        <dd>{baseline.activeProtocolCount}</dd>
        <span>Current saved plans</span>
      </div>
      <div className={`${styles.kpi} ${styles.kpiWide}`}>
        <dt>Last protocol change</dt>
        {baseline.lastProtocolChangeDate ? <>
          <dd className={styles.kpiText}>{baseline.lastProtocolChangeTitle || 'Protocol updated'}</dd>
          <time dateTime={baseline.lastProtocolChangeDate}>{formatTimelineDate(baseline.lastProtocolChangeDate)}</time>
        </> : <dd className={styles.kpiText}>No changes recorded yet</dd>}
      </div>
    </dl>
    {baseline.activeProtocols.length > 0 && <details className={styles.disclosure}>
      <summary>View active protocols <span>{baseline.activeProtocolCount}</span></summary>

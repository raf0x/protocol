...
export default function TodayOverview(props: Props) {
  return <div className="today-overview">
    ...
    <TodaysFocusCard key={props.date} activeCount={props.protocols.length} due={props.due} logs={props.logs} saving={props.saving} onTaken={props.onTaken} error={props.error}>{props.checkin}</TodaysFocusCard>
    {props.schedule}
    <div className="today-insights-grid"><RecentChangesCard events={recentChanges(props.events, props.protocols, props.date)} /><HealthTrendsCard entries={props.entries} unit={props.weightUnit} onToggleUnit={props.onToggleUnit} /></div>

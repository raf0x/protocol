...
type Props = { mood: number | null; energy: number | null; hunger: number | null; sleep: string; weight: string; notes: string; saving: boolean; saved: boolean; onMoodChange: (v: number) => void; onEnergyChange: (v: number) => void; onHungerChange: (v: number) => void; onSleepChange: (v: string) => void; onWeightChange: (v: string) => void; onNotesChange: (v: string) => void; onSave: () => void }
export default function CompactDailyLog({ mood, energy, hunger, sleep, weight, notes, saving, saved, onMoodChange, onEnergyChange, onHungerChange, onSleepChange, onWeightChange, onNotesChange, onSave }: Props) {
  const [openTab, setOpenTab] = useState<string | null>(null)
  const g = 'var(--color-green)', dg = 'var(--color-dim)', mg = 'var(--color-muted)', cb = 'var(--color-card)', bd = 'var(--color-border)'
  const tabs = [{ id: 'mood', label: 'Mood', value: mood, onChange: onMoodChange, reverse: false },{ id: 'energy', label: 'Energy', value: energy, onChange: onEnergyChange, reverse: false },{ id: 'hunger', label: 'Hunger', value: hunger, onChange: onHungerChange, reverse: true }]
  function ScoreBtn({ value, current, onChange, reverse }: { value: number; current: number | null; onChange: (v: number) => void; reverse?: boolean }) {
    ...
    return <button onClick={() => { onChange(value); setOpenTab(null) }} style={{width:'32px',height:'32px',borderRadius:'50%',border:a?'none':'1px solid '+bd,background:a?sc:cb,color:a?'#fff':dg,fontSize:'12px',fontWeight:'700',cursor:'pointer',opacity:a?1:0.5}}>{value}</button>
  }
  const getStatusColor = (val: number | null) => { if (val === null) return mg; if (val >= 4) return '#22c55e'; if (val >= 3) return '#eab308'; return '#ef4444' }
  return (
    <div style={{background:cb,border:'1px solid '+bd,borderRadius:'12px',padding:'14px',marginBottom:'16px'}}>
      ...
      <div style={{display:'flex',gap:'6px',marginBottom:'10px',flexWrap:'wrap'}}>
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setOpenTab(openTab === tab.id ? null : tab.id)} style={{flex:'1 1 0',minWidth:'90px',background:openTab === tab.id ? 'var(--color-surface)' : cb,border:'1px solid '+(openTab === tab.id ? g : bd),borderRadius:'8px',padding:'8px 10px',cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:'4px'}}>
            <span style={{fontSize:'11px',fontWeight:'600',color:'var(--color-text)'}}>{tab.label}</span>
            {tab.value !== null ? <span style={{fontSize:'14px',fontWeight:'700',color:getStatusColor(tab.value)}}>{tab.value}</span> : <span style={{fontSize:'12px',color:mg}}>—</span>}
          </button>
        ))}
      </div>
      {openTab && tabs.find(t => t.id === openTab) && (
        <div style={{background:'var(--color-surface)',border:'1px solid '+bd,borderRadius:'8px',padding:'12px',marginBottom:'10px',display:'flex',gap:'8px',justifyContent:'center'}}>

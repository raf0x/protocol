...
export default function HealthDashboard() {
  ...
  return <main className={styles.page}>
    <header className={styles.header}><span className={styles.eyebrow}>Your health, over time</span><h1>{adding ? 'Add lab results' : panelId ? 'Lab panel' : 'Health'}</h1><p>Lab results and check-ins, in one place.</p>
      <div className={styles.headerLinks}><Link href="/health">Labs</Link><Link href="/journal">Journal history</Link>{!adding && <Link className={styles.primary} href="/health?action=add">Add lab results</Link>}</div>
    </header>
    ...
    {status === 'ready' && (adding ? <AddLabForm onCancel={() => router.push('/health')} onSaved={id => { setSaved(true); retry(); router.push(`/health?panel=${encodeURIComponent(id)}`) }} /> : panelId ? panel ? <>
      <section className={styles.card}><span className={styles.eyebrow}>Manual entry</span><h2>{panel.panel_name || 'Lab results'}</h2><time dateTime={panel.test_date}>{formatTimelineDate(panel.test_date)}</time>{panel.provider && <p>{panel.provider}</p>}<p className={styles.summary}>{panelSummary(panel.results)}</p>{panel.notes && <details className={styles.formDetails}><summary>Panel notes</summary><p className={styles.notes}>{panel.notes}</p></details>}</section>
      <section className={styles.card} aria-label="Biomarker results"><h2>Results</h2><p className={styles.caption}>Status reflects the supplied lab interpretation or numeric reference bounds. It is not a diagnosis.</p>{panel.results.length ? panel.results.map(result => <LabResultRow key={result.id} result={result} />) : <p>No results recorded in this panel.</p>}</section>

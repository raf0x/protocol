...
export default function DashboardPage() {
  ...
  async function loadAll() {
    ...
    const { data: protocols } = await supabase.from('protocols').select('id, start_date, name, notes, compounds(id, name, vial_strength, vial_unit, bac_water_ml, reconstitution_date, doses_taken_override, ml_per_dose, phases(dose, dose_unit, frequency, day_of_week, days_of_week, start_week, end_week, name, time_of_day))').eq('status', 'active')
...
<div style={{ display: 'grid', gridTemplateColumns: '110px 1fr', gap: '8px', marginBottom: '16px' }}>
  <StatsBoxes

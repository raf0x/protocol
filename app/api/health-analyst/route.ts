...
import { INVALID_ANALYST_ACTION, parseGuidedAnalystAction } from '../../../lib/health/analyst/actions'
...
export async function POST(request: NextRequest) {
  ...
  let action = null
  try { action = parseGuidedAnalystAction(await request.json()) } catch { /* Invalid JSON is an invalid action payload. */ }
  if (!action) return NextResponse.json({ code: INVALID_ANALYST_ACTION, error: 'Choose a supported Analyst action.' }, { status: 400 })

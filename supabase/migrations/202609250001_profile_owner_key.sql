-- Correct the existing account-deletion caller for id-owned profiles.
-- No column, policy, owner or grant changes. Existing function ACL is retained.
-- Production confirms profiles.id is the auth.users(id) foreign-key primary key.
-- Exclude profiles only from the user_id loop; keep its explicit id deletion.
BEGIN;
